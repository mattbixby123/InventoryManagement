...
const CardPurchaseSummary = () => {
  ...
  // Calculate total purchased across all days
  const totalPurchased = purchaseData.reduce(
    (acc, curr) => acc + curr.totalPurchased, 
    0
  ) || 0;

  // Calculate average change percentage
  const averageChangePercentage = purchaseData.reduce(
    (acc, curr, _, array) => {
  ...
  const reversedData = purchaseData.slice().reverse();

  const chartData = {
    labels: reversedData.map(() => ''),
    datasets: [
      {
        data: reversedData.map(item => item.totalPurchased),
        borderColor: '#8884d8',
      ...
      tooltip: {
        ...
        callbacks: {
          title: function(context: { dataIndex: number }[]) {
            ...
            const date = new Date(reversedData[index].date);
